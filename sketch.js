...
function draw(){
  ...
  if(gameState === 3){
    ...
    if(mousePressedOver(beg) && gameState === 3){
      gameState = 4;
    }

    if(mousePressedOver(med)&& gameState === 3){
      gameState = 5;
    }

    if(mousePressedOver(dif)&& gameState === 3){
      gameState = 6;
    }
  ...
  if(gameState === 4){
    ...
    if(lives === 0){
      ...
      if(mousePressedOver(home) && gameState === 4){
        gameState = 1
        background(bg);
        lives =3
      }

      if(mousePressedOver(button) && gameState === 4){
        gameState = 3;
        background(bg);
        title.visible=true;
        home.visible=false;
  ...
  if(gameState === 5){
    ...
    if(lives === 0){
      ...
      if(mousePressedOver(home) && gameState === 5){
        gameState = 1
        background(bg);
        lives =3
      }

      if(mousePressedOver(button) && gameState === 5){
        gameState = 3;
        background(bg);
        title.visible=true;
        home.visible=false;
  ...
  if(gameState === 6){
    ...
    if(lives === 0){
      ...
      if(mousePressedOver(home) && gameState === 6){
        gameState = 1
        background(bg);
        lives =3;
        score === 0;
      ...
      if(mousePressedOver(button) && gameState === 6){
        gameState = 3;
        background(bg);
        title.visible=true;
        home.visible=false;
...
function spawnParachute3(){
  if(frameCount % 50 === 0){
    var parachute=createSprite(displayWidth/2, 160);
    parachute.x= Math.round(random(30,displayWidth-50));
    parachute.addImage(parachuteImg);
    parachute.scale=0.7
    parachute.velocityY=10;
    plane1.x=parachute.x;
    plane1.depth=parachute.depth;
    plane1.depth+=1
    parachuteGroup.add(parachute);
